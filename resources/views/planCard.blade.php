<div style="width: 270px;" class="card themed-block text-center py-4 px-1 mb-5 mx-auto d-flex flex-column align-items-center @if($plan->highlighted) highlighted-plan @endif" style="height: 75vh">
    ...
    <form id="paymentForm{{$plan->id}}">
        ...
        <button type="submit" class="btn color-white themed-btn mt-3 position-absolute">
            Seleccionar
        </button>
    </form>
</div>
...
@push('scripts')
    <script>
        document.getElementById("paymentForm{{$plan->id}}").addEventListener("submit", submitListener, true);

        function submitListener(event) {
            event.preventDefault();
            // Si el usuario no está autenticado, redirigir al login
            @auth()
                showPayModal(@json($plan), document.querySelector('.payment-options'));
            @else
